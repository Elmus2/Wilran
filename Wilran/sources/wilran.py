...
def calculate_move_damage(pokemon, move_data, ability_mod=0, is_crit=False):
    """Calculate damage for a move including ability modifier, level scaling, and critical hits"""
    ...
    # Add ability modifier (MOVE modifier) - this is NOT doubled on crits
    total_damage = total_dice_damage + ability_mod

    # Return all components including crit info
    return base_damage, total_damage, damage_type, dice_str, crit_damage
...
def attack_roll(pokemon, move_id):
    ...
    # Determine best ability modifier
    chosen_ability = None
    highest_mod = None
    power_abilities = move_data.get("power")

    if isinstance(power_abilities, list):
        for ability in power_abilities:
            score = ability_scores.get(ability, 10)
            mod = ability_modifier(score)
            if highest_mod is None or mod > highest_mod:
                highest_mod = mod
    ...
    damage_calc = calculate_move_damage(
        pokemon, move_data, highest_mod if highest_mod else 0, is_crit
    )

    if damage_calc[0] is not None:
        base_damage, total_damage, damage_type, dice_str, crit_damage = damage_calc
    else:
        base_damage = total_damage = damage_type = dice_str = crit_damage = None

    # Format results in Roll20 style with cleaner breakdown
    if "save" in description_text:
        total = 8 + (highest_mod if highest_mod is not None else 0) + prof
        ...
        if base_damage is not None:
            if is_crit and crit_damage > 0:
                damage_result = f"Damage on failed save: {total_damage} {damage_type} [CRITICAL! {dice_str}: {base_damage} + {dice_str}: {crit_damage} + {highest_mod} {ability_name}]"
            else:
        ...
        total = d20_roll + \
            (highest_mod if highest_mod is not None else 0) + prof
        ability_name = chosen_ability.upper() if chosen_ability else "N/A"
...
def format_message(pokemon_name, move_name, attack_result, damage_result=None):
    ...
    if "Saving Throw DC:" in attack_result:
        # Extract DC and breakdown
        dc_match = re.search(
            r"Saving Throw DC: (\d+) \[(.*?)\]", attack_result)
        if dc_match:
            dc_value = dc_match.group(1)
            breakdown = dc_match.group(2)
    ...
    def display_pokemon(self, pokemon):
        ...
        if any(pokemon.get(field) for field in ["moves", "abilities"]):
            ...
            if pokemon.get("abilities"):
                tk.Label(moves_frame, text=f"Abilities: {pokemon.get('abilities', '')}",
                         wraplength=wrap_len, justify="left").pack(anchor="w", pady=2)
...
class BattlerFrame(ttk.Frame):
    def __init__(self, parent, battle_log=None):
        super().__init__(parent)
...
def main_gui():
    ...
    battler_panel = BattlerFrame(battler_frame_container, battle_log)
